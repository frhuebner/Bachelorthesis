...
To calculate the Wightmanfunction after the collapse we will follow the treatment of Hawking (see Appendix \todo{link}) to find that the expectation value of two operators thereafter is given by the thermal expectation value with \(\beta = 8\pi M\) in the spacetime before the collapse. The corresponding temperature \(T_H = \frac{1}{8\pi M k\ind{B}}\) is called Hawking temperature.
...
Up to a prefactor this is the same as for an inertial observer in Minkowskispace \todo{link}. So a static observer indeed does not recognize any particles.
...
which results in an observed temperature of \(T = f(r)^{-1/2} T\ind{H}\) which agrees with \todo{link}. So a static observer will see a slightly higher temperature than the Hawking temperature.
...
Clearly \(\tau = 0\) is a root (which will lie in the upper half by \todo{link}). Apart from that we have to differentiate between two cases\todo{more information}:
\begin{itemize}
\item \(\xi < 1\): In this case there are another two roots on the real axis. The reason is that for \(\xi < 1\) (which is a very fast circular motion) the trajectory hits the Minkowski-lightcone. But we know that this is not possible in the Schwarzschild-spacetime by the argumentation of \todo{link}. So we assume that this behaviour is due to our approximation and therefore exclude this case\footnote{Note that \(\xi < 1\) for a geodesic can only happen for \(r \lesssim 1.1 R\ind{S}\) which is definitely not far away from the black hole}.

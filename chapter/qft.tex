...
We will later often choose a basis of positiv frequency solutions. A solution $u$ has positiv frequency w.r.t a futuredirected timelike vectorfield \(k\) if  \(i\nabla_k u = \omega u, \omega > 0\). This means, that \(u(t, y^i) = \frac{\sqrt{2\omega}}e^{-i\omega t} A(y^i)\) where \(t\) is the corresponding coordinate to \(k\). It's not possible to find positiv frequency solutions for any timelike vectorfield, since they also need to satisfy \(\nabla^\mu\nabla_\mu u = 0\). In case of \(k\) killing however it is always possible to find a orthonormal basis of positive frequency solutions (see Townsend). Using the normalisation condition on a cauchysurface \(\Sigma\) with normal proportional to \(\vb{k}\) one finds that:
\begin{align}
u_{\omega, i}(t, \va{y}) &= \frac{1}{\sqrt{2\omega}}e^{-i\omega t} A_i(\va{y})\\
\label{equ:solutions_static}
\delta_{ij} &= \int_\Sigma	\dd{S} \frac{1}{\sqrt{\qty|g_{00}|}} A_i^*(\va{y}) A_j(\va{y})
\end{align}
...
The Klein-Gordon-Equation in Minkowskispace is just the wave equation:
\begin{align}
...
\end{align}
The prefactor \(\frac{1}{\sqrt{2 |k|}}\) is required for normalisation \((u_{\vec{k}}|u_{\vec{k}'}) = \delta^3(\vec{k}-\vec{k}')\). It is natural to define the vacuum in minkowskispace by \(a_{\vec{k}}\ket{0_M} = 0\).

Since the Minkowskispace is also spherical symmetric on can also choose spherical modes
\begin{align}
...
The deltafunction is always zero because \(E \leq 0\), \(\omega > 0\), and \(\alpha > 0\). So the observer does not detect any particles. qed.
